...
from project.models import NewsItem, RecyclingEvent, RecyclingStreet
...
class RecyclingStreetList(Resource):
    def get(self):
        ...
        if now.year > 2021:
            ...
            items = RecyclingStreet.query.filter(
                func.length(RecyclingStreet.town_id) < 5
            ).all()  # < 5 bedeutet TownId bis 2021
...
rest_api.add_resource(NewsItemList, "/newsitems")
rest_api.add_resource(RecyclingStreetList, "/recycling/streets")
rest_api.add_resource(RecyclingEventList, "/recycling/street/<street_id>/events")
